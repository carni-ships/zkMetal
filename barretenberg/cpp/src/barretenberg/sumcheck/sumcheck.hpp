...
#include "barretenberg/ecc/scalar_multiplication/metal/metal_prover_ops.hpp"
...
template <typename Flavor> class SumcheckProver {
  public:
                                   ...
                                   const FF& round_challenge)
    {
        ...
        // GPU zero-copy partial_evaluate: disabled — causes SIGSEGV on large circuits (428K+ gates).
        // Root cause: Metal newBufferWithBytesNoCopy requires page-aligned pointers and page-multiple
        // lengths, which not all polynomial allocations satisfy. Savings were only ~7ms.
        // Code preserved in metal_batch_fold_zero_copy() for future investigation.
