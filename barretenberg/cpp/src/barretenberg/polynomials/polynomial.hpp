...
template <typename Fr> class Polynomial {
  public:
    ...
    /**
     * @brief Shiftable polynomial without zero-initialization.
     * @details Only safe when ALL backing elements will be overwritten before use (e.g. wire/permutation polynomials
     * that are fully populated by trace construction).
     */
    static Polynomial shiftable_uninitialized(size_t size, size_t virtual_size)
    {
        return Polynomial(
            /*actual size*/ size - NUM_ZERO_ROWS, virtual_size, /*shiftable offset*/ NUM_ZERO_ROWS, DontZeroMemory::FLAG);
    }
    ...
    /**
     * @brief Out-of-place factor_roots: reads from this polynomial, writes (p(X) - eval) / (X - root) to dst.
     * @details Avoids copying the polynomial before factoring, saving ~32MB memcpy for large polynomials.
     */
    void factor_roots_to(Polynomial& dst, const Fr& root, const Fr& evaluation) const
    {
        const size_t src_size = size();
        polynomial_arithmetic::factor_roots_out_of_place(
            std::span<const Fr>(data(), src_size), std::span<Fr>(dst.data(), dst.size()), root, evaluation);
        // Zero elements beyond source size (factor_roots reduces degree by 1, output has src_size-1 nonzero elements)
        // dst[src_size-1] is already zeroed by factor_roots_out_of_place, but elements [src_size, dst.size()) may be uninitialized
        if (dst.size() > src_size) {
            memset(static_cast<void*>(dst.data() + src_size), 0, sizeof(Fr) * (dst.size() - src_size));
        }
    };
    ...
    // Page-rounded allocation size in bytes (for Metal zero-copy buffers)
    size_t backing_allocated_bytes() const { return coefficients_.backing_allocated_bytes(); }
