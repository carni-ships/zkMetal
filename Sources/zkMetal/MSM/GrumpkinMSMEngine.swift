// Grumpkin Metal MSM Engine -- Pippenger's bucket method with GPU acceleration
// Grumpkin base field = BN254 Fr, scalar field = BN254 Fq (Fp)

import Foundation
import Metal

public class GrumpkinMSM {
    public static let version = Versions.msmGrumpkin
    ...
    public init() throws {
        ...
        let cacheFile = GrumpkinMSM.cacheDir.appendingPathComponent("grumpkin_msm.metallib")

        let requiredKernels = [
            "grumpkin_msm_reduce_sorted_buckets", "grumpkin_msm_bucket_sum_direct",
            "grumpkin_msm_combine_segments", "grumpkin_signed_digit_extract"
        ]
        if FileManager.default.fileExists(atPath: cacheFile.path) {
            do {
                ...
                if requiredKernels.allSatisfy({ cached.makeFunction(name: $0) != nil }) {
                    ...
                    library = try GrumpkinMSM.compileAndCache(device: device, cacheFile: cacheFile)
                }
            } catch {
                library = try GrumpkinMSM.compileAndCache(device: device, cacheFile: cacheFile)
            }
        ...
        guard let reduceSortedFn = library.makeFunction(name: "grumpkin_msm_reduce_sorted_buckets"),
              let reduceCoopFn = library.makeFunction(name: "grumpkin_msm_reduce_cooperative"),
              let sumDirectFn = library.makeFunction(name: "grumpkin_msm_bucket_sum_direct"),
              let combineFn = library.makeFunction(name: "grumpkin_msm_combine_segments"),
              let hornerFn = library.makeFunction(name: "grumpkin_msm_horner_combine"),
              let signedDigitFn = library.makeFunction(name: "grumpkin_signed_digit_extract"),
              let gpuSortHistFn = library.makeFunction(name: "grumpkin_sort_histogram"),
              let gpuSortScatFn = library.makeFunction(name: "grumpkin_sort_scatter"),
              let gpuBuildCsmFn = library.makeFunction(name: "grumpkin_build_csm") else {
    ...
    private static func compileAndCache(device: MTLDevice, cacheFile: URL) throws -> MTLLibrary {
        ...
        let curveSource = try String(contentsOfFile: shaderDir + "/geometry/grumpkin_curve.metal", encoding: .utf8)
        let msmSource = try String(contentsOfFile: shaderDir + "/msm/grumpkin_msm_kernels.metal", encoding: .utf8)

        func stripIncludes(_ s: String) -> String {
            s.split(separator: "\n").filter { !$0.contains("#include") }.joined(separator: "\n")
        }
        func stripGuards(_ s: String) -> String {
            s.replacingOccurrences(of: "#ifndef BN254_FR_METAL", with: "")
             .replacingOccurrences(of: "#define BN254_FR_METAL", with: "")
             .replacingOccurrences(of: "#endif // BN254_FR_METAL", with: "")
             .replacingOccurrences(of: "#ifndef GRUMPKIN_CURVE_METAL", with: "")
             .replacingOccurrences(of: "#define GRUMPKIN_CURVE_METAL", with: "")
             .replacingOccurrences(of: "#endif // GRUMPKIN_CURVE_METAL", with: "")
        }

        let combined = stripGuards(frSource) + "\n" +
        ...
        try? FileManager.default.createDirectory(
            at: GrumpkinMSM.cacheDir, withIntermediateDirectories: true)

        if #available(macOS 11.0, *) {
            let archiveDesc = MTLBinaryArchiveDescriptor()
            if let archive = try? device.makeBinaryArchive(descriptor: archiveDesc) {
                for name in ["grumpkin_msm_reduce_sorted_buckets", "grumpkin_msm_bucket_sum_direct"] {
    ...
    // Grumpkin scalar field order = BN254 base field Fq as 8x32-bit limbs (little-endian)
    ...
    private func ensureBuffers(n: Int, nBuckets: Int, nSegments: Int, nWindows: Int) {
        ...
        if needRealloc {
            ...
            pointsBuffer = device.makeBuffer(
                length: MemoryLayout<GrumpkinPointAffine>.stride * np, options: .storageModeShared)
            ...
            bucketsBuffer = device.makeBuffer(
                length: MemoryLayout<GrumpkinPointProjective>.stride * nb * nw, options: .storageModeShared)
            segmentResultsBuffer = device.makeBuffer(
                length: MemoryLayout<GrumpkinPointProjective>.stride * ns * nw, options: .storageModeShared)
            windowResultsBuffer = device.makeBuffer(
                length: MemoryLayout<GrumpkinPointProjective>.stride * nw, options: .storageModeShared)
            finalResultBuffer = device.makeBuffer(
                length: MemoryLayout<GrumpkinPointProjective>.stride, options: .storageModeShared)
    ...
    public func msm(points: [GrumpkinPointAffine], scalars: [[UInt32]]) throws -> GrumpkinPointProjective {
        let n = points.count
        ...
        let gpuPtsPtr = pointsBuffer.contents().bindMemory(to: GrumpkinPointAffine.self, capacity: effectiveN)
        ...
        do {
            ...
            enc.setBytes(&params, length: MemoryLayout<GrumpkinMsmParams>.stride, index: 4)
        ...
        do {
            ...
            enc.setBytes(&params, length: MemoryLayout<GrumpkinMsmParams>.stride, index: 2)
        ...
        let winResultsPtr = windowResultsBuffer.contents().bindMemory(to: GrumpkinPointProjective.self, capacity: nWindows)
        var windowResults = [GrumpkinPointProjective](repeating: grumpkinPointIdentity(), count: nWindows)
        for w in 0..<nWindows {
        ...
        for w in stride(from: nWindows - 2, through: 0, by: -1) {
            for _ in 0..<windowBits {
                result = grumpkinPointDouble(result)
            }
            result = grumpkinPointAdd(result, windowResults[w])
        }
...
// GrumpkinMsmParams must match Metal struct layout
public struct GrumpkinMsmParams {
