// BLS12-381 G2 Metal MSM Engine -- Pippenger's bucket method with GPU acceleration
// G2 points have Fp2 coordinates (4x wider than G1), scalars are Fr381.
// Used for Groth16 batch verification and BLS signature aggregation.

import Foundation
import Metal

/// GPU point types matching the Metal shader layout.
/// G2Point381AffineGPU: 4 Fp381 elements = 4 * 48 bytes = 192 bytes
/// G2Point381ProjectiveGPU: 6 Fp381 elements = 6 * 48 bytes = 288 bytes
public struct G2Point381AffineGPU {
    public var x_c0: Fp381
    public var x_c1: Fp381
    public var y_c0: Fp381
    public var y_c1: Fp381

    public init(from p: G2Affine381) {
        self.x_c0 = p.x.c0
        self.x_c1 = p.x.c1
        self.y_c0 = p.y.c0
        self.y_c1 = p.y.c1
    ...
    public func toG2Affine() -> G2Affine381 {
        G2Affine381(x: Fp2_381(c0: x_c0, c1: x_c1), y: Fp2_381(c0: y_c0, c1: y_c1))
    }
}

public struct G2Point381ProjectiveGPU {
    public var x_c0: Fp381
    public var x_c1: Fp381
    public var y_c0: Fp381
    public var y_c1: Fp381
    public var z_c0: Fp381
    public var z_c1: Fp381

    public func toG2Projective() -> G2Projective381 {
        G2Projective381(
            x: Fp2_381(c0: x_c0, c1: x_c1),
            y: Fp2_381(c0: y_c0, c1: y_c1),
            z: Fp2_381(c0: z_c0, c1: z_c1))
    }
}
...
public class BLS12381G2MSM {
    public static let version = Versions.msmBLS12381G2
    ...
    public init() throws {
        ...
        let cacheFile = BLS12381G2MSM.cacheDir.appendingPathComponent("bls12381_g2_msm.metallib")

        let requiredKernels = [
            "g2_msm381_reduce_sorted_buckets", "g2_msm381_bucket_sum_direct",
            "g2_msm381_combine_segments", "g2_msm381_signed_digit_extract"
        ]
        if FileManager.default.fileExists(atPath: cacheFile.path) {
            do {
                ...
                if requiredKernels.allSatisfy({ cached.makeFunction(name: $0) != nil }) {
                    ...
                    library = try BLS12381G2MSM.compileAndCache(device: device, cacheFile: cacheFile)
                }
            } catch {
                library = try BLS12381G2MSM.compileAndCache(device: device, cacheFile: cacheFile)
            }
        ...
        guard let reduceSortedFn = library.makeFunction(name: "g2_msm381_reduce_sorted_buckets"),
              let sumDirectFn = library.makeFunction(name: "g2_msm381_bucket_sum_direct"),
              let combineFn = library.makeFunction(name: "g2_msm381_combine_segments"),
              let hornerFn = library.makeFunction(name: "g2_msm381_horner_combine"),
              let signedDigitFn = library.makeFunction(name: "g2_msm381_signed_digit_extract"),
              let gpuSortHistFn = library.makeFunction(name: "g2_msm381_sort_histogram"),
              let gpuSortScatFn = library.makeFunction(name: "g2_msm381_sort_scatter"),
              let gpuBuildCsmFn = library.makeFunction(name: "g2_msm381_build_csm") else {
    ...
    private static func compileAndCache(device: MTLDevice, cacheFile: URL) throws -> MTLLibrary {
        ...
        let msmSource = try String(contentsOfFile: shaderDir + "/msm/bls12381_g2_msm_kernels.metal", encoding: .utf8)
        ...
        try? FileManager.default.createDirectory(
            at: BLS12381G2MSM.cacheDir, withIntermediateDirectories: true)

        if #available(macOS 11.0, *) {
            let archiveDesc = MTLBinaryArchiveDescriptor()
            if let archive = try? device.makeBinaryArchive(descriptor: archiveDesc) {
                for name in ["g2_msm381_reduce_sorted_buckets", "g2_msm381_bucket_sum_direct"] {
    ...
    // r = 0x73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000000
    ...
    private func ensureBuffers(n: Int, nBuckets: Int, nSegments: Int, nWindows: Int) {
        ...
        if needRealloc {
            ...
            pointsBuffer = device.makeBuffer(
                length: MemoryLayout<G2Point381AffineGPU>.stride * np, options: .storageModeShared)
            ...
            bucketsBuffer = device.makeBuffer(
                length: MemoryLayout<G2Point381ProjectiveGPU>.stride * nb * nw, options: .storageModeShared)
            segmentResultsBuffer = device.makeBuffer(
                length: MemoryLayout<G2Point381ProjectiveGPU>.stride * ns * nw, options: .storageModeShared)
            windowResultsBuffer = device.makeBuffer(
                length: MemoryLayout<G2Point381ProjectiveGPU>.stride * nw, options: .storageModeShared)
            finalResultBuffer = device.makeBuffer(
                length: MemoryLayout<G2Point381ProjectiveGPU>.stride, options: .storageModeShared)
            ...
            // Scratch arrays for count-of-counts during CSM building
    ...
    /// Points are G2 affine (Fp2 coordinates), scalars are Fr381 as 8x UInt32 limbs.
    public func msm(points: [G2Affine381], scalars: [[UInt32]]) throws -> G2Projective381 {
        let n = points.count
        ...
        // Fr381 scalar bits = 255
        let scalarBits = 255

        // Window sizing: smaller windows for G2 due to 4x wider coordinates
        ...
        let gpuPtsPtr = pointsBuffer.contents().bindMemory(to: G2Point381AffineGPU.self, capacity: n)
        for i in 0..<n {
            gpuPtsPtr[i] = G2Point381AffineGPU(from: points[i])
        }
        ...
        do {
            ...
            enc.setBytes(&params, length: MemoryLayout<G2Msm381Params>.stride, index: 4)
        ...
        do {
            ...
            enc.setBytes(&params, length: MemoryLayout<G2Msm381Params>.stride, index: 2)
        ...
        let winResultsPtr = windowResultsBuffer.contents().bindMemory(to: G2Point381ProjectiveGPU.self, capacity: nWindows)
        ...
        for w in stride(from: nWindows - 2, through: 0, by: -1) {
            for _ in 0..<windowBits {
                result = g2_381Double(result)
            }
            let wr = winResultsPtr[w].toG2Projective()
            if !g2_381IsIdentity(wr) {
                result = g2_381IsIdentity(result) ? wr : g2_381Add(result, wr)
            }
        }
    ...
    /// CPU fallback MSM for small inputs (double-and-add)
    private func cpuMSM(points: [G2Affine381], scalars: [[UInt32]]) -> G2Projective381 {
        let n = points.count
        ...
        for i in 0..<n {
            ...
            let p = g2_381ScalarMul(g2_381FromAffine(points[i]), u64)
            result = g2_381IsIdentity(result) ? p : g2_381Add(result, p)
        }
        return result
    }
}
