// Pallas Metal MSM Engine — Pippenger's bucket method with GPU acceleration

import Foundation
import Metal

public class PallasMSM {
    public static let version = Versions.msmPallas
    ...
    public init() throws {
        ...
        let cacheFile = PallasMSM.cacheDir.appendingPathComponent("pallas_msm.metallib")

        let requiredKernels = [
            "pallas_msm_reduce_sorted_buckets", "pallas_msm_bucket_sum_direct",
            "pallas_msm_combine_segments", "pallas_signed_digit_extract"
        ]
        if FileManager.default.fileExists(atPath: cacheFile.path) {
            do {
                ...
                if requiredKernels.allSatisfy({ cached.makeFunction(name: $0) != nil }) {
                    ...
                    library = try PallasMSM.compileAndCache(device: device, cacheFile: cacheFile)
                }
            } catch {
                library = try PallasMSM.compileAndCache(device: device, cacheFile: cacheFile)
            }
        ...
        guard let reduceSortedFn = library.makeFunction(name: "pallas_msm_reduce_sorted_buckets"),
              let reduceCoopFn = library.makeFunction(name: "pallas_msm_reduce_cooperative"),
              let sumDirectFn = library.makeFunction(name: "pallas_msm_bucket_sum_direct"),
              let combineFn = library.makeFunction(name: "pallas_msm_combine_segments"),
              let hornerFn = library.makeFunction(name: "pallas_msm_horner_combine"),
              let signedDigitFn = library.makeFunction(name: "pallas_signed_digit_extract"),
              let gpuSortHistFn = library.makeFunction(name: "pallas_sort_histogram"),
              let gpuSortScatFn = library.makeFunction(name: "pallas_sort_scatter"),
              let gpuBuildCsmFn = library.makeFunction(name: "pallas_build_csm") else {
    ...
    private static func compileAndCache(device: MTLDevice, cacheFile: URL) throws -> MTLLibrary {
        ...
        let fpSource = try String(contentsOfFile: shaderDir + "/fields/pallas_fp.metal", encoding: .utf8)
        let curveSource = try String(contentsOfFile: shaderDir + "/geometry/pallas_curve.metal", encoding: .utf8)
        let msmSource = try String(contentsOfFile: shaderDir + "/msm/pallas_msm_kernels.metal", encoding: .utf8)

        func stripIncludes(_ s: String) -> String {
            s.split(separator: "\n").filter { !$0.contains("#include") }.joined(separator: "\n")
        }
        func stripGuards(_ s: String) -> String {
            s.replacingOccurrences(of: "#ifndef PALLAS_FP_METAL", with: "")
             .replacingOccurrences(of: "#define PALLAS_FP_METAL", with: "")
             .replacingOccurrences(of: "#endif // PALLAS_FP_METAL", with: "")
             .replacingOccurrences(of: "#ifndef PALLAS_CURVE_METAL", with: "")
             .replacingOccurrences(of: "#define PALLAS_CURVE_METAL", with: "")
             .replacingOccurrences(of: "#endif // PALLAS_CURVE_METAL", with: "")
        }
        ...
        try? FileManager.default.createDirectory(
            at: PallasMSM.cacheDir, withIntermediateDirectories: true)

        if #available(macOS 11.0, *) {
            let archiveDesc = MTLBinaryArchiveDescriptor()
            if let archive = try? device.makeBinaryArchive(descriptor: archiveDesc) {
                for name in ["pallas_msm_reduce_sorted_buckets", "pallas_msm_bucket_sum_direct"] {
    ...
    private static func findShaderDir() -> String {
        ...
        for bundle in Bundle.allBundles {
            if let url = bundle.url(forResource: "Shaders", withExtension: nil) {
                let path = url.appendingPathComponent("fields/pallas_fp.metal").path
        ...
        for path in candidates {
            if FileManager.default.fileExists(atPath: "\(path)/fields/pallas_fp.metal") {
                return path
            }
        }
    ...
    // Pallas scalar field order (= Vesta base field) as 8x32-bit limbs (little-endian)
    ...
    private func ensureBuffers(n: Int, nBuckets: Int, nSegments: Int, nWindows: Int) {
        ...
        if needRealloc {
            ...
            pointsBuffer = device.makeBuffer(
                length: MemoryLayout<PallasPointAffine>.stride * np, options: .storageModeShared)
            ...
            bucketsBuffer = device.makeBuffer(
                length: MemoryLayout<PallasPointProjective>.stride * nb * nw, options: .storageModeShared)
            segmentResultsBuffer = device.makeBuffer(
                length: MemoryLayout<PallasPointProjective>.stride * ns * nw, options: .storageModeShared)
            windowResultsBuffer = device.makeBuffer(
                length: MemoryLayout<PallasPointProjective>.stride * nw, options: .storageModeShared)
            finalResultBuffer = device.makeBuffer(
                length: MemoryLayout<PallasPointProjective>.stride, options: .storageModeShared)
    ...
    public func msm(points: [PallasPointAffine], scalars: [[UInt32]]) throws -> PallasPointProjective {
        let n = points.count
        ...
        let gpuPtsPtr = pointsBuffer.contents().bindMemory(to: PallasPointAffine.self, capacity: effectiveN)
        ...
        do {
            ...
            enc.setBytes(&params, length: MemoryLayout<PallasMsmParams>.stride, index: 4)
        ...
        do {
            ...
            enc.setBytes(&params, length: MemoryLayout<PallasMsmParams>.stride, index: 2)
        ...
        let winResultsPtr = windowResultsBuffer.contents().bindMemory(to: PallasPointProjective.self, capacity: nWindows)
        var windowResults = [PallasPointProjective](repeating: pallasPointIdentity(), count: nWindows)
        for w in 0..<nWindows {
        ...
        for w in stride(from: nWindows - 2, through: 0, by: -1) {
            for _ in 0..<windowBits {
                result = pallasPointDouble(result)
            }
            result = pallasPointAdd(result, windowResults[w])
        }
...
// PallasMsmParams must match Metal struct layout
public struct PallasMsmParams {
