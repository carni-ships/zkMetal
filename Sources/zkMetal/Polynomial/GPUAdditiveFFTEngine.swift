...
public class GPUAdditiveFFTEngine {
    ...
    public let forwardSimdFn: MTLComputePipelineState?  // SIMD-optimized variant (uchar4, 4 elements/thread)
    public let forwardSimd8Fn: MTLComputePipelineState?  // SIMD variant (8 elements/thread)
    ...
    public enum KernelVariant {
        ...
        case simd              // n/4-thread SIMD kernel (4 elements per thread) - RECOMMENDED
        case simd8             // n/8-thread SIMD kernel (8 elements per thread)
    }

    /// Auto-select optimal kernel variant based on transform size.
    public func forwardAuto(data: [UInt8], n: Int, k: Int, basis: [UInt8]) throws -> [UInt8] {
        // For large FFTs (n >= 2^16), use SIMD variant
        if n >= 65536 && forwardSimdFn != nil {
            print("Using SIMD kernel for n=\(n)...")
            return try forwardSimd(data: data, n: n, k: k, basis: basis)
        }
        // For medium FFTs, use pairs kernel
        else if forwardPairsFn != nil {
            return try forwardPairs(data: data, n: n, k: k, basis: basis)
        }
        // Fallback to standard
        return try forward(data: data, n: n, k: k, basis: basis)
    }
    ...
    public init() throws {
        ...
        let kernelNames = [
            ...
            "additive_fft_gf8_forward_simd",
            "additive_fft_gf8_forward_simd8",
        ...
        if let pipelines = try? cache.loadOrCompile(
            ...
            self.forwardSimdFn = pipelines["additive_fft_gf8_forward_simd"]  // SIMD uchar4 kernel (4x improvement)
            self.forwardSimd8Fn = pipelines["additive_fft_gf8_forward_simd8"]  // SIMD uchar4 kernel (8 elements per thread)
            ...
            self.forwardSimdFn = nil
            self.forwardSimd8Fn = nil
    ...
    /// Forward additive FFT using n/2 threads (one per butterfly pair).
    /// All threads are active at all levels, eliminating divergence.
    /// Each thread processes one butterfly pair per level.
    public func forwardPairs(data: [UInt8], n: Int, k: Int, basis: [UInt8]) throws -> [UInt8] {
        precondition(data.count == n, "Data must have exactly n elements")
        precondition(basis.count == k, "Basis must have exactly k elements")
        precondition(n == (1 << k), "n must equal 2^k")
    ...
    /// Forward additive FFT using SIMD vectorization (uchar4).
    /// Each thread processes 4 consecutive elements, providing 4x better memory coalescing.
    /// Target: Large FFTs (n >= 4096) where SIMD benefits outweigh kernel complexity.
    ///
    /// Expected speedup: ~4x over standard kernel due to:
    /// - Reduced thread count (n/4 instead of n)
    /// - Better memory coalescing (adjacent threads access adjacent memory)
    /// - Higher GPU occupancy
    public func forwardSimd(data: [UInt8], n: Int, k: Int, basis: [UInt8]) throws -> [UInt8] {
        precondition(data.count == n, "Data must have exactly n elements")
        precondition(basis.count == k, "Basis must have exactly k elements")
        precondition(n == (1 << k), "n must equal 2^k")
        ...
        guard let fn = forwardSimdFn else {
            throw MSMError.gpuError("Forward SIMD pipeline not available (Metal shader compilation failed)")
        }
        ...
        enc.setBytes(&kVal, length: 4, index: 4)
        let tg = min(tuning.nttThreadgroupSize, Int(fn.maxTotalThreadsPerThreadgroup))
        // n/4 threads since each thread processes 4 elements
        let nQuads = n / 4
        enc.dispatchThreads(MTLSize(width: nQuads, height: 1, depth: 1),
                           threadsPerThreadgroup: MTLSize(width: tg, height: 1, depth: 1))
        enc.endEncoding()
    ...
    /// Forward additive FFT using SIMD vectorization (uchar4, 8 elements per thread).
    /// Even more aggressive vectorization than forwardSimd.
    /// Target: Very large FFTs (n >= 2^20) where thread count is limiting factor.
    public func forwardSimd8(data: [UInt8], n: Int, k: Int, basis: [UInt8]) throws -> [UInt8] {
        precondition(data.count == n, "Data must have exactly n elements")
        precondition(basis.count == k, "Basis must have exactly k elements")
        precondition(n == (1 << k), "n must equal 2^k")
        ...
        guard let fn = forwardSimd8Fn else {
            throw MSMError.gpuError("Forward SIMD8 pipeline not available (Metal shader compilation failed)")
        }
        ...
        enc.setBytes(&kVal, length: 4, index: 4)
        let tg = min(tuning.nttThreadgroupSize, Int(fn.maxTotalThreadsPerThreadgroup))
        // n/8 threads since each thread processes 8 elements
        let nOcts = n / 8
        enc.dispatchThreads(MTLSize(width: nOcts, height: 1, depth: 1),
                           threadsPerThreadgroup: MTLSize(width: tg, height: 1, depth: 1))
        enc.endEncoding()
    ...
    /// Forward additive FFT using SIMD shuffle optimization.
    /// Both upper and lower elements participate in each butterfly level.
    /// Uses simd_shuffle to exchange partner values within each warp,
    /// eliminating one uncoalesced global memory read per butterfly level.
    /// Tradeoff: compute doubles (both sides do twist-propagate) but memory traffic halves.
    public func forwardShuffle(data: [UInt8], n: Int, k: Int, basis: [UInt8]) throws -> [UInt8] {
        precondition(data.count == n, "Data must have exactly n elements")
        precondition(basis.count == k, "Basis must have exactly k elements")
        precondition(n == (1 << k), "n must equal 2^k")
        ...
        guard let fn = forwardShuffleFn else {
            throw MSMError.gpuError("Forward shuffle pipeline not available (Metal shader compilation failed)")
        }
