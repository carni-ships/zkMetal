// NTT Engine — GPU-accelerated Number Theoretic Transform on BLS12-377 Fr
...
public class BLS12377NTTEngine {
    public static let version = Versions.nttBLS12377
    ...
    public init() throws {
        ...
        let library = try BLS12377NTTEngine.compileShaders(device: device)

        guard let butterflyFn = library.makeFunction(name: "bls377_ntt_butterfly"),
              let butterflyRadix4Fn = library.makeFunction(name: "bls377_ntt_butterfly_radix4"),
              let invButterflyFn = library.makeFunction(name: "bls377_intt_butterfly"),
              let invButterflyRadix4Fn = library.makeFunction(name: "bls377_intt_butterfly_radix4"),
              let butterflyFusedFn = library.makeFunction(name: "bls377_ntt_butterfly_fused"),
              let invButterflyFusedFn = library.makeFunction(name: "bls377_intt_butterfly_fused"),
              let scaleFn = library.makeFunction(name: "bls377_ntt_scale"),
              let bitrevFn = library.makeFunction(name: "bls377_ntt_bitrev"),
              let bitrevInplaceFn = library.makeFunction(name: "bls377_ntt_bitrev_inplace"),
              let columnFusedFn = library.makeFunction(name: "bls377_ntt_column_fused"),
              let rowFusedFn = library.makeFunction(name: "bls377_ntt_row_fused"),
              let twiddleMultiplyFn = library.makeFunction(name: "bls377_ntt_twiddle_multiply"),
              let transposeFn = library.makeFunction(name: "bls377_ntt_transpose"),
              let invColumnFusedFn = library.makeFunction(name: "bls377_intt_column_fused"),
              let invColumnFusedTwiddleFn = library.makeFunction(name: "bls377_intt_column_fused_twiddle"),
              let invRowFusedFn = library.makeFunction(name: "bls377_intt_row_fused"),
              let rowFusedTwiddleFn = library.makeFunction(name: "bls377_ntt_row_fused_twiddle"),
              let rowFusedTwiddleTransposeFn = library.makeFunction(name: "bls377_ntt_row_fused_twiddle_transpose"),
              let butterflyFusedBitrevFn = library.makeFunction(name: "bls377_ntt_butterfly_fused_bitrev"),
              let columnFusedSubblockFn = library.makeFunction(name: "bls377_ntt_column_fused_subblock"),
              let columnButterflyFn = library.makeFunction(name: "bls377_ntt_column_butterfly"),
              let columnButterflyRadix4Fn = library.makeFunction(name: "bls377_ntt_column_butterfly_radix4"),
              let transposeRectFn = library.makeFunction(name: "bls377_ntt_transpose_rect"),
              let invColumnButterflyFn = library.makeFunction(name: "bls377_intt_column_butterfly"),
              let invColumnButterflyRadix4Fn = library.makeFunction(name: "bls377_intt_column_butterfly_radix4"),
              let invColumnFusedSubblockTwiddleFn = library.makeFunction(name: "bls377_intt_column_fused_subblock"),
              let invRowFusedTwiddleFn = library.makeFunction(name: "bls377_intt_row_fused_twiddle") else {
    ...
    private static func compileShaders(device: MTLDevice) throws -> MTLLibrary {
        ...
        let frSource = try String(contentsOfFile: shaderDir + "/fields/bls12377_fr.metal", encoding: .utf8)
        let nttSource = try String(contentsOfFile: shaderDir + "/ntt/ntt_bls12377.metal", encoding: .utf8)
    ...
    private static func findNTTShaderDir() -> String {
        ...
        for bundle in Bundle.allBundles {
            if let url = bundle.url(forResource: "Shaders", withExtension: nil) {
                let frPath = url.appendingPathComponent("fields/bls12377_fr.metal").path
        ...
        for path in candidates {
            let frPath = "\(path)/fields/bls12377_fr.metal"
            if FileManager.default.fileExists(atPath: frPath) {
                return path
            }
        }
    ...
    private func getScratchBuffer(n: Int) -> MTLBuffer {
        let needed = n * MemoryLayout<Fr377>.stride
    ...
    private func getTwiddles(logN: Int) -> MTLBuffer {
        ...
        let twiddles = precomputeTwiddles377(logN: logN)
    ...
    private func getInvTwiddles(logN: Int) -> MTLBuffer {
        ...
        let twiddles = precomputeInverseTwiddles377(logN: logN)
        let buf = createFrBuffer(twiddles)!
    ...
    private func getInvN(logN: Int) -> MTLBuffer {
        ...
        let n = UInt64(1 << logN)
        let invN = fr377Inverse(fr377FromInt(n))
        let buf = createFrBuffer([invN])!
        invNCache[logN] = buf
        return buf
    }

    /// Create a Metal buffer from an array of Fr377 elements.
    private func createFrBuffer(_ data: [Fr377]) -> MTLBuffer? {
        let byteCount = data.count * MemoryLayout<Fr377>.stride
    ...
    // Max elements per threadgroup for fused kernel (1024 Fr377 * 32 bytes = 32KB shared mem)
    ...
    public func ntt(data: MTLBuffer, logN: Int) throws {
        let globalStages = logN - BLS12377NTTEngine.maxFusedLogN
        ...
        let fusedStages = min(logN, BLS12377NTTEngine.maxFusedLogN)
        ...
        if hasFused {
            ...
            blit.copy(from: scratch!, sourceOffset: 0, to: data, destinationOffset: 0, size: nInt * MemoryLayout<Fr377>.stride)
            blit.endEncoding()
        }
    ...
    public func intt(data: MTLBuffer, logN: Int) throws {
        ...
        if fusedStages > 1 {
            ...
            enc.setThreadgroupMemoryLength((1 << fusedStages) * MemoryLayout<Fr377>.stride, index: 0)
            enc.dispatchThreadgroups(MTLSize(width: numGroups, height: 1, depth: 1),
                                   threadsPerThreadgroup: MTLSize(width: tgThreads, height: 1, depth: 1))
        }
    ...
    private func nttFourStep(data: MTLBuffer, logN: Int) throws {
        ...
        let logN2: Int
        let logN1: Int
        if logN <= 2 * BLS12377NTTEngine.maxFusedLogN {
            logN1 = (logN + 1) / 2
            logN2 = logN - logN1
        } else {
            logN2 = BLS12377NTTEngine.maxFusedLogN
            logN1 = logN - logN2
        }
        let n1 = UInt32(1 << logN1)
        let n2 = UInt32(1 << logN2)

        let colFusedStages = min(logN1, BLS12377NTTEngine.maxFusedLogN)
    ...
    /// High-level NTT: takes Fr377 array, returns NTT'd array.
    /// Uses cached buffer to avoid per-call Metal allocation overhead.
    public func ntt(_ input: [Fr377]) throws -> [Fr377] {
        let n = input.count
        precondition(n > 0 && (n & (n - 1)) == 0, "NTT size must be power of 2")
        let logN = Int(log2(Double(n)))
        let stride = MemoryLayout<Fr377>.stride
        ...
        let ptr = dataBuf.contents().bindMemory(to: Fr377.self, capacity: n)
        return Array(UnsafeBufferPointer(start: ptr, count: n))
    }

    /// High-level iNTT: takes NTT'd array, returns original coefficients.
    /// Uses cached buffer to avoid per-call Metal allocation overhead.
    public func intt(_ input: [Fr377]) throws -> [Fr377] {
        let n = input.count
        precondition(n > 0 && (n & (n - 1)) == 0, "NTT size must be power of 2")
        let logN = Int(log2(Double(n)))
        let stride = MemoryLayout<Fr377>.stride
    ...
    public static func cpuNTT(_ input: [Fr377], logN: Int) -> [Fr377] {
        let n = input.count
        ...
        for s in 0..<logN {
            ...
            k = stepsToOmega
            while k > 0 {
                if k & 1 == 1 { w_m = fr377Mul(w_m, temp) }
                temp = fr377Sqr(temp)
                k >>= 1
            }
            ...
            for block in stride(from: 0, to: n, by: blockSize) {
                ...
                for j in 0..<halfBlock {
                    let u = data[block + j]
                    let v = fr377Mul(w, data[block + j + halfBlock])
                    data[block + j] = fr377Add(u, v)
                    data[block + j + halfBlock] = fr377Sub(u, v)
                    w = fr377Mul(w, w_m)
                }
            }
        }
    ...
    /// CPU reference iNTT.
    public static func cpuINTT(_ input: [Fr377], logN: Int) -> [Fr377] {
        let n = input.count
        let omega = fr377RootOfUnity(logN: logN)
        ...
        for si in 0..<logN {
            ...
            for block in stride(from: 0, to: n, by: blockSize) {
                ...
                for j in 0..<halfBlock {
                    ...
                    data[block + j] = fr377Add(a, b)
                    data[block + j + halfBlock] = fr377Mul(fr377Sub(a, b), w)
                    w = fr377Mul(w, w_m)
                }
            }
        }
        ...
        // Scale by 1/n
        let invN = fr377Inverse(fr377FromInt(UInt64(n)))
        for i in 0..<n {
            data[i] = fr377Mul(data[i], invN)
        }
        return data
    }

    /// Bit-reversal permutation.
    private static func bitReverse(_ data: [Fr377], logN: Int) -> [Fr377] {
        let n = data.count
        var result = data
        for i in 0..<n {
            var rev = 0
