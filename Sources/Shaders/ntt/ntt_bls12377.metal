// NTT/iNTT GPU kernels for BLS12-377 scalar field
...
#include "../fields/bls12377_fr.metal"
...
kernel void bls377_ntt_butterfly(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles      [[buffer(1)]],
...
kernel void bls377_intt_butterfly(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles_inv  [[buffer(1)]],
    constant uint& n               [[buffer(2)]],
    constant uint& stage           [[buffer(3)]],
    uint gid                       [[thread_position_in_grid]]
) {
...
kernel void bls377_ntt_butterfly_radix4(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles      [[buffer(1)]],
    constant uint& n               [[buffer(2)]],
    constant uint& stage           [[buffer(3)]],
    ...
    data[base + h]     = fr377_add(b1, wb3);
    data[base + 3 * h] = fr377_sub(b1, wb3);
}

// Radix-4 DIF butterfly for BN254 iNTT: processes 2 stages at once.
kernel void bls377_intt_butterfly_radix4(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles_inv  [[buffer(1)]],
    constant uint& n               [[buffer(2)]],
    constant uint& stage           [[buffer(3)]],
...
kernel void bls377_ntt_butterfly_fused(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles      [[buffer(1)]],
    constant uint& n               [[buffer(2)]],
    ...
    threadgroup Fr377 shared[1024]; // max 1024 Fr377 elements = 32KB
    if (base + tid < n)
...
kernel void bls377_ntt_butterfly_fused_bitrev(
    device const Fr377* input         [[buffer(0)]],   // source data (natural order)
    device Fr377* output              [[buffer(1)]],   // destination (after bitrev + fused stages)
    device const Fr377* twiddles      [[buffer(2)]],
...
kernel void bls377_intt_butterfly_fused(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles_inv  [[buffer(1)]],
    constant uint& n               [[buffer(2)]],
...
kernel void bls377_ntt_scale(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* scale_factor  [[buffer(1)]],
    constant uint& n               [[buffer(2)]],
    uint gid                       [[thread_position_in_grid]]
) {
    if (gid >= n) return;
    data[gid] = fr377_mul(data[gid], scale_factor[0]);
}

// --- Bit-reversal permutation kernel ---

kernel void bls377_ntt_bitrev(
    device const Fr377* input         [[buffer(0)]],
    device Fr377* output              [[buffer(1)]],
    constant uint& n               [[buffer(2)]],
...
kernel void bls377_ntt_bitrev_inplace(
    device Fr377* data                [[buffer(0)]],
...
kernel void bls377_ntt_column_fused(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles      [[buffer(1)]],
...
kernel void bls377_ntt_row_fused(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles      [[buffer(1)]],
...
kernel void bls377_ntt_row_fused_twiddle(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles      [[buffer(1)]],
    constant uint& n               [[buffer(2)]],    // total size N
    constant uint& local_stages    [[buffer(3)]],    // log2(N2)
    ...
    // row = tgid, col = idx
    if (base + idx_lo < n) {
        Fr377 val = data[base + idx_lo];
        uint tw_idx = (uint)((ulong(tgid) * ulong(idx_lo)) % ulong(n));
        if (tw_idx != 0) val = fr377_mul(val, twiddles[tw_idx]);
        shared[rev_lo] = val;
    }
    if (base + idx_hi < n) {
        Fr377 val = data[base + idx_hi];
...
kernel void bls377_ntt_row_fused_twiddle_transpose(
    device const Fr377* input         [[buffer(0)]],
    device Fr377* output              [[buffer(1)]],
    device const Fr377* twiddles      [[buffer(2)]],
    ...
    // Load with twiddle multiply: val *= omega_N^(row * col)
    if (base + idx_lo < n) {
        Fr377 val = input[base + idx_lo];
        uint tw_idx = (uint)((ulong(tgid) * ulong(idx_lo)) % ulong(n));
        if (tw_idx != 0) val = fr377_mul(val, twiddles[tw_idx]);
        shared[rev_lo] = val;
    }
    if (base + idx_hi < n) {
        Fr377 val = input[base + idx_hi];
        uint tw_idx = (uint)((ulong(tgid) * ulong(idx_hi)) % ulong(n));
        if (tw_idx != 0) val = fr377_mul(val, twiddles[tw_idx]);
        shared[rev_hi] = val;
    }
...
kernel void bls377_ntt_twiddle_multiply(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles      [[buffer(1)]],
...
kernel void bls377_intt_column_fused_twiddle(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles_inv  [[buffer(1)]],
    constant uint& n               [[buffer(2)]],    // total size N
    constant uint& n1              [[buffer(3)]],    // N1 (column size)
    constant uint& n2              [[buffer(4)]],    // N2 (number of columns)
    constant uint& local_stages    [[buffer(5)]],    // log2(N1)
    device const Fr377* inv_n         [[buffer(6)]],    // 1/N in Montgomery form
    uint tid                       [[thread_index_in_threadgroup]],
    uint tgid                      [[threadgroup_position_in_grid]],
    uint tg_size                   [[threads_per_threadgroup]]
) {
    ...
    if (tid < n1) {
        Fr377 val = data[col + tid * n2];
        uint tw_idx = (uint)((ulong(tid) * ulong(col)) % ulong(n));
        if (tw_idx != 0) val = fr377_mul(val, twiddles_inv[tw_idx]);
        shared[tid] = val;
    }
    if (tid + tg_size < n1) {
        uint row = tid + tg_size;
        Fr377 val = data[col + row * n2];
    ...
    uint rev_hi = bitrev(tid + tg_size, local_stages);
    if (tid < n1)
        data[col + tid * n2] = fr377_mul(shared[rev_lo], scale);
    if (tid + tg_size < n1)
        data[col + (tid + tg_size) * n2] = fr377_mul(shared[rev_hi], scale);
}

// DIF row iFFT: load row naturally, DIF stages, write back bit-reversed
kernel void bls377_intt_row_fused(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles_inv  [[buffer(1)]],
    constant uint& n               [[buffer(2)]],    // total size N
    constant uint& local_stages    [[buffer(3)]],    // log2(N2)
...
kernel void bls377_ntt_column_fused_subblock(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles      [[buffer(1)]],
    constant uint& n               [[buffer(2)]],    // total size N
...
kernel void bls377_ntt_column_butterfly(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles      [[buffer(1)]],
...
kernel void bls377_ntt_column_butterfly_radix4(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles      [[buffer(1)]],
    constant uint& n1              [[buffer(2)]],    // column size (N1)
    constant uint& n2              [[buffer(3)]],    // stride (N2)
    ...
    data[addr1] = fr377_add(b1, wb3);
    data[addr3] = fr377_sub(b1, wb3);
}

// Out-of-place rectangular transpose: input[r*cols+c] → output[c*rows+r]
kernel void bls377_ntt_transpose_rect(
    device const Fr377* input         [[buffer(0)]],
    device Fr377* output              [[buffer(1)]],
...
kernel void bls377_intt_column_butterfly(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles_inv  [[buffer(1)]],
    constant uint& n1              [[buffer(2)]],    // column size (N1)
    constant uint& n2              [[buffer(3)]],    // stride (N2)
...
kernel void bls377_intt_column_butterfly_radix4(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles_inv  [[buffer(1)]],
    constant uint& n1              [[buffer(2)]],    // column size (N1)
    constant uint& n2              [[buffer(3)]],    // stride (N2)
    ...
    Fr377 sum02 = fr377_add(a0, a2);
    Fr377 diff02 = fr377_sub(a0, a2);
    Fr377 sum13 = fr377_add(a1, a3);
...
kernel void bls377_intt_column_fused_subblock(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles_inv  [[buffer(1)]],
    constant uint& n               [[buffer(2)]],    // total size N
    ...
    constant uint& num_subblocks   [[buffer(6)]],    // N1 / sub_size
    device const Fr377* inv_n         [[buffer(7)]],    // 1/N in Montgomery form
    uint tid                       [[thread_index_in_threadgroup]],
    uint tgid                      [[threadgroup_position_in_grid]],
    uint tg_size                   [[threads_per_threadgroup]]
) {
    ...
    uint rev_hi = bitrev(tid + tg_size, local_stages);
    if (tid < sub_size)
        data[col + (sub_base + tid) * n2] = fr377_mul(shared[rev_lo], scale);
    if (tid + tg_size < sub_size)
        data[col + (sub_base + tid + tg_size) * n2] = fr377_mul(shared[rev_hi], scale);
}
...
kernel void bls377_intt_row_fused_twiddle(
    device Fr377* data                [[buffer(0)]],
    device const Fr377* twiddles_inv  [[buffer(1)]],
    constant uint& n               [[buffer(2)]],    // total size N
    constant uint& local_stages    [[buffer(3)]],    // log2(N2)
...
kernel void bls377_ntt_transpose(
    device Fr377* data                [[buffer(0)]],
