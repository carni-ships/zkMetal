// Pallas Fr NTT GPU kernels
// Pallas scalar field Fr = Vesta base field Fp (cycle property)
// Uses VestaFp arithmetic for all field operations.
// Cooley-Tukey radix-2 DIT forward, Gentleman-Sande radix-2 DIF inverse.

#include "../fields/vesta_fp.metal"

// Type alias for clarity: NTT operates on Pallas Fr elements = VestaFp
typedef VestaFp PallasFr;

inline PallasFr pfr_add(PallasFr a, PallasFr b) { return vesta_add(a, b); }
inline PallasFr pfr_sub(PallasFr a, PallasFr b) { return vesta_sub(a, b); }
inline PallasFr pfr_mul(PallasFr a, PallasFr b) { return vesta_mul(a, b); }

// Helper: compute bit-reversal of val with num_bits bits
inline uint pallas_bitrev(uint val, uint num_bits) {
    uint result = 0;
...
kernel void pallas_ntt_butterfly(
    device PallasFr* data           [[buffer(0)]],
    device const PallasFr* twiddles [[buffer(1)]],
    ...
    if (twiddle_idx == 0) {
        ...
        PallasFr w = twiddles[twiddle_idx];
        PallasFr wb = pfr_mul(w, b);
        data[i] = pfr_add(a, wb);
        data[j] = pfr_sub(a, wb);
    }
}
...
kernel void pallas_intt_butterfly(
    device PallasFr* data              [[buffer(0)]],
    device const PallasFr* twiddles_inv [[buffer(1)]],
    constant uint& n                    [[buffer(2)]],
    constant uint& stage                [[buffer(3)]],
    uint gid                            [[thread_position_in_grid]]
) {
    ...
    PallasFr sum = pfr_add(a, b);
    PallasFr diff = pfr_sub(a, b);
    data[i] = sum;
    if (twiddle_idx == 0) {
        data[j] = diff;
    } else {
        data[j] = pfr_mul(diff, twiddles_inv[twiddle_idx]);
    }
}

// --- Fused NTT: multiple DIT stages in threadgroup memory ---
kernel void pallas_ntt_butterfly_fused(
    device PallasFr* data           [[buffer(0)]],
    device const PallasFr* twiddles [[buffer(1)]],
    constant uint& n                [[buffer(2)]],
    ...
    threadgroup PallasFr shared[1024];
    if (base + tid < n)
    ...
    for (uint s = 0; s < local_stages; s++) {
        ...
        PallasFr b = shared[j];
        if (twiddle_idx == 0) {
            shared[i] = pfr_add(a, b);
            shared[j] = pfr_sub(a, b);
        } else {
            PallasFr w = twiddles[twiddle_idx];
            PallasFr wb = pfr_mul(w, b);
            shared[i] = pfr_add(a, wb);
            shared[j] = pfr_sub(a, wb);
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
...
kernel void pallas_ntt_butterfly_fused_bitrev(
    device const PallasFr* input    [[buffer(0)]],
    device PallasFr* output         [[buffer(1)]],
    device const PallasFr* twiddles [[buffer(2)]],
    ...
    if (global_lo < n)
        shared[idx_lo] = input[pallas_bitrev(global_lo, logN)];
    if (global_hi < n)
        shared[idx_hi] = input[pallas_bitrev(global_hi, logN)];
    threadgroup_barrier(mem_flags::mem_threadgroup);
...
kernel void pallas_intt_butterfly_fused(
    device PallasFr* data                  [[buffer(0)]],
    device const PallasFr* twiddles_inv    [[buffer(1)]],
    constant uint& n                        [[buffer(2)]],
    constant uint& local_stages             [[buffer(3)]],
    ...
    for (uint si = 0; si < local_stages; si++) {
        ...
        if (twiddle_idx == 0) {
            ...
            shared[j] = pfr_mul(diff, twiddles_inv[twiddle_idx]);
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
...
kernel void pallas_ntt_bitrev_inplace(
    device PallasFr* data   [[buffer(0)]],
    ...
    if (gid >= n) return;
    uint rev = pallas_bitrev(gid, logN);
    if (gid < rev) {
        PallasFr tmp = data[gid];
        data[gid] = data[rev];
        data[rev] = tmp;
    }
}
...
// --- Scale by 1/n ---
kernel void pallas_ntt_scale(
    device PallasFr* data          [[buffer(0)]],
    device const PallasFr* inv_n   [[buffer(1)]],
    constant uint& n               [[buffer(2)]],
    uint gid                       [[thread_position_in_grid]]
) {
    if (gid >= n) return;
    data[gid] = pfr_mul(data[gid], inv_n[0]);
}

// --- Bit-reversal + scale (combined for iNTT final step) ---
kernel void pallas_ntt_bitrev_scale(
    device PallasFr* data          [[buffer(0)]],
    device const PallasFr* inv_n   [[buffer(1)]],
    constant uint& n               [[buffer(2)]],
    ...
    uint rev = pallas_bitrev(gid, logN);
    if (gid <= rev) {
        PallasFr a = pfr_mul(data[gid], inv_n[0]);
        PallasFr b = pfr_mul(data[rev], inv_n[0]);
        data[gid] = b;
        data[rev] = a;
    }
}
