// BLS12-377 MSM GPU kernels: Pippenger's bucket method
// Adapted from BN254 MSM kernels for 12-limb Fq377 field elements.

#include "../geometry/bls12377_curve.metal"
...
kernel void msm377_reduce_sorted_buckets(
    ...
    if (orig_bucket == 0) {
        buckets[flat_idx] = point377_identity();
        return;
    }
...
// SIMD shuffle helper for Point377Projective (12 limbs per field element)
inline Point377Projective simd_shuffle_down_point377(Point377Projective p, uint offset) {
    Point377Projective r;
    for (int k = 0; k < FQ_LIMBS; k++) {
...
kernel void msm377_reduce_cooperative(
    device const Point377Affine* points    [[buffer(0)]],
    device Point377Projective* buckets     [[buffer(1)]],
    device const uint* bucket_offsets      [[buffer(2)]],
    device const uint* bucket_counts       [[buffer(3)]],
    ...
    if (orig_bucket == 0 || bucket_counts[flat_idx] == 0) {
        if (lid == 0) buckets[flat_idx] = point377_identity();
        return;
    }
    ...
    for (uint off = 16; off > 0; off >>= 1) {
        Point377Projective other = simd_shuffle_down_point377(acc, off);
        if (lid < off) {
            if (point377_is_identity(acc)) {
                acc = other;
            } else if (!point377_is_identity(other)) {
                acc = point377_add(acc, other);
            }
        }
    }
...
kernel void msm377_bucket_sum_direct(
    device const Point377Projective* buckets   [[buffer(0)]],
    device Point377Projective* segment_results [[buffer(1)]],
    constant Msm377Params& params              [[buffer(2)]],
    ...
    if (lo_s < 1) lo_s = 1;
    if (hi_s <= lo_s) {
        segment_results[tid] = point377_identity();
        return;
    }

    Point377Projective running = point377_identity();
    Point377Projective sum = point377_identity();

    uint hi = uint(hi_s);
    uint lo = uint(lo_s);
    for (uint i = hi - 1; i >= lo; i--) {
        Point377Projective bucket = buckets[bucket_base + i];
        if (!point377_is_identity(bucket)) {
            if (point377_is_identity(running)) {
                running = bucket;
            } else {
                running = point377_add(running, bucket);
            }
        }
        if (!point377_is_identity(running)) {
            if (point377_is_identity(sum)) {
                sum = running;
            } else {
                sum = point377_add(sum, running);
            }
        }
        if (i == lo) break;
    }
    ...
    uint weight = lo - 1;
    if (weight > 0 && !point377_is_identity(running)) {
        Point377Projective weighted = point377_identity();
        Point377Projective base = running;
        uint k = weight;
        while (k > 0) {
            if (k & 1u) {
                if (point377_is_identity(weighted)) {
                    weighted = base;
                } else {
                    weighted = point377_add(weighted, base);
                }
            }
            base = point377_double(base);
            k >>= 1;
        }
        if (point377_is_identity(sum)) {
            sum = weighted;
        } else {
            sum = point377_add(sum, weighted);
        }
    }
...
kernel void msm377_combine_segments(
    device const Point377Projective* segment_results [[buffer(0)]],
    device Point377Projective* window_results        [[buffer(1)]],
    ...
    for (uint s = 0; s < n_segments; s++) {
        Point377Projective seg = segment_results[base + s];
        if (!point377_is_identity(seg)) {
            if (point377_is_identity(sum)) {
                sum = seg;
            } else {
                sum = point377_add(sum, seg);
            }
        }
    }
...
kernel void msm377_horner_combine(
    device const Point377Projective* window_results [[buffer(0)]],
    device Point377Projective* final_result         [[buffer(1)]],
    ...
    Point377Projective result = window_results[n_windows - 1];
    for (int w = int(n_windows) - 2; w >= 0; w--) {
        for (uint b = 0; b < window_bits; b++) {
            result = point377_double(result);
        }
        Point377Projective wr = window_results[w];
        if (!point377_is_identity(wr)) {
            if (point377_is_identity(result)) {
                result = wr;
            } else {
                result = point377_add(result, wr);
            }
        }
    }
...
kernel void msm377_sort_histogram(
...
kernel void msm377_sort_scatter(
    device const uint* digits          [[buffer(0)]],
