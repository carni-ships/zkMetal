...
template <typename FF> class Selector {
  public:
    ...
    /**
     * @brief Pre-allocate capacity to avoid repeated reallocations.
...
template <typename FF, size_t NUM_WIRES_> class ExecutionTraceBlock {
  public:
    ...
    /**
     * @brief Pre-allocate capacity for wires and selectors to avoid repeated reallocations.
     * @param num_gates Expected number of gates in this block.
     */
    void reserve(size_t num_gates)
    {
        for (auto& wire : wires) {
            wire.reserve(num_gates);
        }
        for (auto& sel : get_selectors()) {
            sel.reserve(num_gates);
        }
    }
